...
## Correlated t-test wrapper
def correlated_ttest_w_sampling(x, rope, title,runs=1, verbose=False, names=('C1', 'C2'), nsamples=50000, save_fig=False):
    '''
    Wrapper to call 'correlated_ttest' & 'correlated_tttest_MC' tgr.
    
    '''
    #--- 1) call 'correlated_ttest'
    left, within, right = correlated_ttest(x=x, rope=rope, runs=runs, verbose=verbose, names=names)

    #--- 2) plot posterior
    
    #generate samples from posterior (it is not necesssary because the posterior is a Student)
    samples=correlated_ttest_MC(x=x, rope=rope,runs=runs,nsamples=nsamples)
    #plot posterior
    ...
    # align x axis
    axes = fig.axes
    axes.set_xlim(-0.07,0.37)
    axes.set_ylim(0,25)
    #add label
    plt.xlabel(names[1]+' vs ' +names[0]+' on '+names[2]+' dataset')
    
    if(save_fig):
        fig.get_figure().savefig(to_dir+title+'.png',dpi=1000,bbox_inches="tight",pad_inches=0)#, bbox_inches='tight', pad_inches=10
